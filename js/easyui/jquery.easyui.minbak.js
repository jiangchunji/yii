...
(function ($) {
    ...
    function _dc(_dd, ul, _de, _df) {
        ...
        function _e3(ul, _e6, _e7) {
            for (var i = 0; i < _e6.length; i++) {
                ...
                _e9.attr("node-id", _e8.id);//为treenode节点添加属性url
                if (typeof (_e8.nodeCls) != "undefined") { _e9.addClass(_e8.nodeCls); } //为treenode节点添加样式
                $.data(_e9[0], "tree-node", { id: _e8.id, text: _e8.text, iconCls: _e8.iconCls,url: _e8.url,attributes: _e8.attributes });
    ...
    $.fn.tree.methods = { options: function (jq) {
        ...
        return jq.each(function (index, treetarget) {
            if (_164 != undefined) { $(_164).parent().find("div.tree-node").each(function (index, target) { _c3(treetarget, target, true); }); }
            else { $(this).find("div.tree-node").each(function (index, target) { _c3(treetarget, target, true); }); }
        });
    }, uncheckall: function (jq, _164) {
        return jq.each(function (index, treetarget) {
            if (_164 != undefined) { $(_164).parent().find("div.tree-node").each(function (index, target) { _c3(treetarget, target, false); }); }
            else { $(this).find("div.tree-node").each(function (index, target) { _c3(treetarget, target, false); }); }
        });
...
(function ($) {
    ...
    function _1a0(_1a1) {
        ...
        if (_1a2.options.href && (!_1a2.isLoaded || !_1a2.options.cache)) {
            ...
            //将提交方式手动改成get
            $.ajax({ url: _1a2.options.href, type: "get", cache: false, success: function (data) {
...
(function ($) {
    ...
    }, length: { validator: function (_39e, _39f) {
        var len = $.trim(_39e).length;
        return len >= _39f[0] && len <= _39f[1];
    }, message: "Please enter a value between {0} and {1}."
    }, remote: { validator: function (_3a0, _3a1) {
        var data = {};
        data[_3a1[1]] = _3a0;
...
(function ($) {
    ...
    function _537(_538, _539) {
        ...
        if (opts.pagination) {//修改pageIndex  pageSize 用于分页参数
            $.extend(_53a, { PageIndex: opts.pageNumber, PageSize: opts.pageSize });
        }
        if (opts.sortName) {
            $.extend(_53a, { SortField: opts.sortName, SortOrder: opts.sortOrder });
        }
...
//修改
//1212 为treenode节点添加属性url和nodeClass
//7571 修改pageIndex  pageSize 用于分页参数
//2125 将提交方式手动改成get 
//1761 添加可用/不可用方法
//1790 全选全不选
//2470 自定义panel方法
//10860 自定义确定事件(datetimebox)